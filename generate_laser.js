...
const TEMP_DIR = '/tmp/laser-variations';
...
const PROMPT = `Energy laser bolt, cyan white glow, elongated beam, sci-fi projectile, game sprite, transparent background, vertical energy beam, bright core with outer glow, 16x32 pixel aspect ratio, clean edges, bioluminescent style`;
...
async function generateVariation(index) {
  ...
  try {
    ...
    const tempFile = path.join(TEMP_DIR, `laser-v${index}.png`);
...
async function resizeAndOptimize(inputFile, outputFile) {
  ...
  try {
    // Resize to 16x32 and optimize
    await execAsync(
      `convert "${inputFile}" -resize 16x32! -strip -define png:compression-level=9 "${outputFile}"`
    );
...
async function main() {
  console.log('🚀 Starting laser.png generation pipeline...');
  ...
  for (const variation of variations) {
    const outputFile = path.join(TEMP_DIR, `laser-v${variation.index}-16x32.png`);
  ...
  const finalOutput = path.join(OUTPUT_DIR, 'laser.png');
