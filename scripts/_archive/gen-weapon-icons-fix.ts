/**
 * Redo shotgun, m1911, and grenade weapon icons.
 */
...
const icons = [
  { name: 'weapon-shotgun', desc: 'Ithaca 37 pump-action shotgun, right-facing side profile' },
  { name: 'weapon-m1911', desc: 'M1911 semi-automatic pistol handgun, right-facing side profile' },
...
for (const icon of icons) {
  ...
  const prompt = `${icon.desc}, white weapon silhouette on flat background, clean simple weapon shape only, no text, no labels, no words, no numbers, ${STYLE_SUFFIX}`;
