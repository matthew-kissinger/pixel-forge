/**
 * Tall grass redo - Vietnam tropical jungle, no yellow
 */
...
const prompt = [
  ...
  'dense tropical grass field ground, top-down overhead view,',
  'thick lush green grass blades, deep tropical greens,',
  'various shades of green from dark emerald to bright jungle green,',
  'no yellow, no brown, no dry grass, wet humid tropical vegetation,',
...
console.log('Regenerating tall-grass (tropical lush green, no yellow)...\n');

const raw = await fluxGenerate(prompt, 256);
if (!raw) { console.log('FAILED'); process.exit(1); }

await Bun.write(`${OUT_DIR}/tall-grass_raw.png`, raw);
...
await Bun.write(`${OUT_DIR}/tall-grass.png`, buf);
