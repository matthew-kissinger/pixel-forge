...
- **Handler tests broken (41 failures)** - `handlers.test.ts` has two issues: (1) `canvas.getContext('2d')` returns null in happy-dom test environment (36 tests), (2) `vi.mocked()` is not a function in Bun (5 batch tests). File is untracked. Fix: either mock canvas or use a canvas polyfill like `canvas`/`@napi-rs/canvas`, and replace `vi.mocked()` with direct `(fn as Mock)` casting.
...
- **Main bundle still 700KB (gzip: 208KB)** - Three.js (380KB gzip) already split into separate chunk, KilnGenNode already lazy-loaded. Remaining 208KB main chunk could benefit from splitting React Flow and heavy node components.
