/**
 * Fix NVA front-fire: barrel at viewer + preserve muzzle flash from BiRefNet.
 * Modified chroma cleanup skips yellow/orange flash pixels.
 */
...
async function chromaCleanMagentaPreserveFlash(buf: Buffer): Promise<Buffer> {
  const { data, info } = await sharp(buf).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  const pixels = new Uint8Array(data.buffer);
  let cleaned = 0;
  for (let i = 0; i < pixels.length; i += 4) {
    const r = pixels[i], g = pixels[i + 1], b = pixels[i + 2];
    // Preserve muzzle flash: yellow/orange/white pixels
  ...
  if (cleaned > 0) console.log(`    Chroma: ${cleaned} px (preserved flash)`);
...
const vcFireFront = loadImageAsBase64(`${GAME_SPRITES}/vc-fire-front.webp`);

console.log('=== Fix NVA front-fire (barrel at viewer, preserve flash) ===\n');

const prompt = `Recreate the second reference image's firing pose with the NVA soldier from the first reference image. ${NVA_UNIFORM}. Front facing view, soldier aiming rifle DIRECTLY AT THE VIEWER. Barrel foreshortened pointing straight at camera. Bright yellow-orange muzzle flash at barrel tip. Firing stance. Full body head to toe, no text, ${STYLE_SUFFIX}`;

console.log('  Generating...');
const gen = await apiPost('image/generate', {
  prompt,
  aspectRatio: '1:1',
  referenceImages: [nvaCharRef, vcFireFront],
});

if (!gen.image) { console.error('FAILED'); process.exit(1); }

const rawB64 = (gen.image as string).replace(/^data:image\/\w+;base64,/, '');
await Bun.write(`${OUT_DIR}/nva-front-fire_raw.png`, Buffer.from(rawB64, 'base64'));
...
const clean = await chromaCleanMagentaPreserveFlash(Buffer.from(bgB64, 'base64'));
await Bun.write(`${OUT_DIR}/nva-front-fire.png`, clean);
