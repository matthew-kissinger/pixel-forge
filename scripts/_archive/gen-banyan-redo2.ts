// Banyan tree redo v2: imposing, can have vines/branches/twist but not overdone
// Should look like a real banyan in 32-bit pixel art game aesthetic
...
const subject = 'Imposing large banyan tree, thick twisted brown trunk, some aerial roots and hanging vines from branches, wide spreading dense dark green leaf canopy, tall and majestic, side view';
...
console.log('Banyan tree redo v2...');
...
if (!genData.image) { console.log(`FAILED: ${genData.error || 'no image'}`); process.exit(1); }

const rawBase64 = genData.image.replace(/^data:image\/\w+;base64,/, '');
const rawBuf = Buffer.from(rawBase64, 'base64');
await Bun.write(`${OUT_DIR}/banyan-tree_raw.png`, rawBuf);
...
if (!bgData.image) { console.log(`BG removal failed: ${bgData.error || 'no image'}`); process.exit(1); }
...
await Bun.write(`${OUT_DIR}/banyan-tree.png`, cleanBuf);
