/**
 * Generate VC mounted/seated soldier sprite.
 * Uses US mounted as pose reference + VC walk-front as faction style reference.
 */
...
const VC_UNIFORM = 'Viet Cong guerrilla fighter: black pajama clothing, conical straw non la hat, ammunition bandolier across chest, minimal web gear';
...
console.log('=== VC Mounted Soldier Sprite ===\n');

// Load references: VC faction style + US mounted pose
const vcFactionRef = loadImageAsBase64(`${GAME_SPRITES}/vc-walk-front-1.webp`);
const usMountedRef = loadImageAsBase64(`${OUT_DIR}/us-mounted.png`);

const prompt = `${VC_UNIFORM}, ${MOUNTED_DESC}, Match the exact pose from the second reference image. front facing view, no text, ${STYLE_SUFFIX}`;

console.log('  Generating...');
const gen = await apiPost('image/generate', {
  prompt,
  aspectRatio: '1:1',
  referenceImages: [vcFactionRef, usMountedRef],
});

if (!gen.image) { console.error('  FAILED'); process.exit(1); }

const rawB64 = (gen.image as string).replace(/^data:image\/\w+;base64,/, '');
await Bun.write(`${OUT_DIR}/vc-mounted_raw.png`, Buffer.from(rawB64, 'base64'));
...
await Bun.write(`${OUT_DIR}/vc-mounted.png`, clean);
